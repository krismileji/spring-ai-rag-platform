    ...
    private Flux<ChatResponse> internalStream(Prompt prompt, ChatResponse previousChatResponse) {
        return Flux.deferContextual(contextView -> {
            ...
            Flux<ChatResponse> chatResponse = ollamaResponse.map(chunk -> {
                ...
                Optional<String> thinkingOpt = Optional.ofNullable(chunk.message()).map(OllamaApi.Message::thinking);
                var assistantMessageBuilder = AssistantMessage.builder()
                        .content(content)
                        .properties(Map.of())
                        .toolCalls(toolCalls);
                thinkingOpt.ifPresent(thinking -> assistantMessageBuilder.properties(Map.of("thinking", thinking)));
                ...
                Optional<String> thinking = Optional.ofNullable(chunk.message()).map(OllamaApi.Message::thinking);
                if (thinking.isPresent()) {
                    generationMetadata = ChatGenerationMetadata.builder()
                            .finishReason(chunk.doneReason())
                            .metadata("thinking", thinking.get())
                            .build();
                }
