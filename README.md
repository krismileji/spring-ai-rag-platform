...
- **🚀 多模型支持**: 集成 DeepSeek、Ollama、Alibaba DashScope 等多种主流大模型，支持多模态与纯文本两种调用方式。
...
1. 克隆项目到本地：
   ```bash
   git clone https://github.com/krismileji/spring-ai-rag-platform.git
   ```
...
3. **通用设置**：切换主题、语言（中文/英文）
4. **关于**：查看系统版本和项目信息

**AI 网页访问**:
- AI 可自主决定是否访问网页获取实时信息
- 支持询问实时新闻、最新文档等问题
- 示例：「帮我查看今天的科技新闻」
...
项目内置了 `visit_web` 工具，AI 可在对话过程中自主调用该工具访问网页获取实时信息。
...
**前端**：使用 Vue I18n，自动检测浏览器语言，用户可在设置页面切换。

**后端**：通过 `Accept-Language` 请求头识别语言，返回对应语言的错误消息和枚举描述。

### 多模态模型支持

支持阿里云 DashScope 的多模态模型调用：
